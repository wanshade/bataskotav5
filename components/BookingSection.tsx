...
const getExpandedSlots = (dayKey: string) => {
  ...
  raw.forEach(item => {
    ...
    // If duration is larger than 1 hour, break into 1-hour slots
    if (duration > 1.05) {
      let current = start;
  ...
  // Dedup based on label (in case of overlaps in source data)
...
• Waktu: ${timeSlotStr} (${selectedSlots.length} jam)
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className={`font-sans font-bold text-lg ${isBooked ? 'text-red-500' : isPending ? 'text-yellow-500' : isSelected ? 'text-neon-green' : 'text-white'
                            }`}>
                            {slot.label}
                          </span>
                          {isBooked ? (
                        ...
                        <div className={`text-sm font-sans ${isBooked ? 'text-red-400 font-bold uppercase' : isPending ? 'text-yellow-400 font-bold uppercase' : isSelected ? 'text-white' : 'text-gray-500'
                          }`}>
                          {isBooked ? 'BOOKED' : isPending ? 'PENDING' : formatPrice(slot.price)}
                        </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-black/50 rounded-lg p-4 space-y-3 border border-zinc-800/50">
                    ...
                    <div className="flex justify-between items-center pt-1">
                      <span className="text-gray-400 text-sm">Total ({selectedSlots.length} jam)</span>
                  ...
                  <form onSubmit={handleBooking} className="space-y-4">
                    ...
                    >
                      {isSubmitting ? 'Memproses...' : `Konfirmasi Pemesanan (${selectedSlots.length} jam)`}
                    </NeonButton>
