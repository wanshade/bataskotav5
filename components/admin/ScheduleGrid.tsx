...
export default function ScheduleGrid({ bookings, loading, onRefresh }: ScheduleGridProps) {
...
return false;
  ...
  const handleApproveBooking = async (bookingId: string) => {
    if (!confirm('Apakah Anda yakin ingin meng-approve booking ini?')) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Gagal approve booking');
      }
      
      setSelectedBooking(null);
      onRefresh?.();
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Gagal approve booking');
    } finally {
  ...
  const handleUpdatePayment = async (bookingId: string, status: 'pending' | 'dp' | 'paid', dpAmount?: number) => {
    setActionLoading(`payment-${status}`);
    try {
      const response = await fetch(`/api/admin/bookings/${bookingId}/payment`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paymentStatus: status, dpAmount }),
      });
      ...
      alert(error instanceof Error ? error.message : 'Gagal update status pembayaran');
    } finally {
      setActionLoading(null);
    }
  };
